...
class RNADataset:
    def __init__(self,rna_graphs,rna_labels,rna_ID,names,smiles=None):
        ...
        for i in range(len(rna_graphs)):
            data=Data(x=rna_graphs[i]['node_feat'],edge_index=rna_graphs[i]['edge_index'],
                      edge_attr=rna_graphs[i]['edge_feat'],y=rna_labels[i])
...
def train_rnas(pdb_rna_path, graph_rna_path):
    ...
    rna_graphs={}
    rna_labels={}
    with open('data/protein_train.csv','r')as f:
        reader=csv.reader(f)
        next(reader)
        ...
        for train_name in rna_graphs.keys():
            train_rna_ID[train_name]=[i for i in range(start_index,start_index+len(rna_graphs[train_name]))]
        ...
        train_set=RNADataset(train_rna_graphs,train_rna_labels,train_rna_ID,train_names)
    f.close()

    rna_graphs={}
    rna_labels={}
    with open('data/protein_inductive.csv','r')as f1:
        ...
        for valid_name in rna_graphs.keys():
            valid_rna_ID[valid_name]=[i for i in range(start_index,start_index+len(rna_graphs[valid_name]))]
        ...
        val_set=RNADataset(valid_rna_graphs,valid_rna_labels,valid_rna_ID,valid_names)
    f1.close()
...
def test_rnas(pdb_rna_path, graph_rna_path):
    rna_graphs = {}
    rna_labels = {}
    with open('data/protein_transductive.csv', 'r') as f1:
        reader = csv.reader(f1)
        next(reader)
        for row in tqdm(reader):
            if row[0] not in rna_graphs.keys():
    ...
    for test_name in rna_graphs.keys():
        test_rna_ID[test_name] = [i for i in range(start_index, start_index + len(rna_graphs[test_name]))]
...
class FewShotBatchSampler:
    def __init__(self, train_rna_datasets, K_shot, K_query, batch_size):
        ...
        for selected_protein in proteins:
            ...
            for index in rna_ID[selected_protein]:
                if self.rna_labels[index] == 1:
                    ...
                    negative_num += 1
            self.ratio_dict[selected_protein] = positive_num / (positive_num + negative_num)
        ...
        while len(proteins) >= self.batch_size:
            ...
            selected_proteins = random.choices(proteins, weights=group_weights, k=self.batch_size)
...
class Test_valBatchSampler:
    ...
    def __iter__(self):
        for protein_name in list(self.rna_ID.keys()):
            ...
            for i in range(n):
                yield self.rna_ID[protein_name][i * self.val_shot:(i + 1) * self.val_shot]
