...
class GCN_DTIMAML(pl.LightningModule):
    ...
    def new_seperate(self, batch):
        ...
        for key, value in protein_index.items():
            ...
            edge_index.extend([shot_batch.edge_index])
            edge_attr.extend([shot_batch.edge_attr])
            node_feat.extend([shot_batch.x])
            label.extend([shot_batch.y])
            batch_batch.extend([shot_batch.batch])
            pdbID.append(key)
